...
- Connectors built so far, easy→hard order: CSV, Excel, MySQL, Postgres, Notion, Slack, Google Drive.
...
- Remaining connectors, easy→hard: Qdrant (pgvector) — architecturally different (semantic query, not row fetch), needs a design decision before starting — then Stripe (mock, Indian signup unavailable), BigQuery (needs sandbox mode re-enabled, billing account was deleted), GA4, Mixpanel, WooCommerce, Salesforce, HubSpot, QuickBooks, Teams, Shopify, Snowflake.
